...
year.addEventListener(`change`, e =>{datosBusqueda.year = parseInt (e.target.value);
    filtrarAuto();
});
...
puertas.addEventListener(`change`, e =>{datosBusqueda.puertas = parseInt (e.target.value);
    filtrarAuto();
});
...
//Genera los años del selec
function llenarSelect(){
    for (let i = max; i>=min; i--){
    const opcion = document.createElement(`option`);
    opcion.value = i;
    opcion.textContent = i;
    year.appendChild(opcion); //Agregar las opciones de años a select
    }
}
